...
export default function ConversationScreen() {
  ...
  const checkedPermissionsForRoom = useRef<number | null>(null);
  ...
  // Re-check delete permission when room data changes (memoize to prevent infinite loops)
  const checkDeletePermissionMemoized = useCallback(async (roomData: ChatRoom) => {
    try {
  ...
  // Re-check delete permission when room data changes (only once per room)
  useEffect(() => {
    if (room && room.id && checkedPermissionsForRoom.current !== room.id) {
      checkedPermissionsForRoom.current = room.id;
      checkDeletePermissionMemoized(room);
    }
  ...
  const loadRoom = async () => {
    try {
      ...
      // Delete permission will be checked by useEffect when room state is set
