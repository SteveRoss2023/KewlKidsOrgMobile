...
export default function OneDriveConnectScreen() {
  ...
  useEffect(() => {
    if (params.success === 'true' && params.service === 'onedrive' && params.message) {
      // Wait a moment for backend to process, then check connection
      const timer = setTimeout(async () => {
        const status = await oauthService.checkConnection('onedrive');
        setConnectionStatus(status);
        if (status.connected) {
          setSuccess(decodeURIComponent(params.message || 'OneDrive connected successfully!'));
          setTimeout(() => {
            setSuccess('');
          }, 2000);
        }
        ...
        if (Platform.OS === 'web') {
          router.replace('/(tabs)/onedrive-connect');
        }
      }, 500);
  ...
  useEffect(() => {
    if (connectionStatus.connected) {
      // Always clear "Please complete authorization" message when connected
      setSuccess((prev) => {
  ...
  // Clear message when screen comes into focus (e.g., from deep link)
  useFocusEffect(
    React.useCallback(() => {
      // When screen is focused, check connection and clear the message if connected
      const checkAndClear = async () => {
        const status = await oauthService.checkConnection('onedrive');
        setConnectionStatus(status);
        if (status.connected) {
          // Force clear "Please complete authorization" message if connected
          setSuccess((prev) => {
            if (prev && prev.includes('Please complete authorization')) {
              return '';
            }
  ...
  const checkConnection = async () => {
    try {
      ...
      if (status.connected) {
        setSuccess((prev) => {
          // Clear immediately if it's the "Please complete authorization" message
          if (prev && prev.includes('Please complete authorization')) {
            return '';
          }
          return prev;
      ...
      if (result.success) {
        ...
        if (status.connected) {
          ...
          if (!result.message.includes('Please complete authorization')) {
            setSuccess(result.message || 'OneDrive connected successfully!');
          }
          // Don't set the "Please complete authorization" message
        }
      } else {
        setError(result.message || 'Failed to connect OneDrive');
      }
    } catch (err: any) {
      console.error('Error connecting OneDrive:', err);
      setError(err.message || 'Failed to connect OneDrive. Please try again.');
    } finally {
      setConnecting(false);
    }
  };
    ...
    >
      ...
      {connectionStatus.connected ? (
        ...
        <View style={[styles.emptyCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <FontAwesome name="cloud" size={48} color={colors.textSecondary} />
          <Text style={[styles.emptyTitle, { color: colors.text }]}>
            OneDrive Not Connected
          </Text>
          <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
            Connect your OneDrive account to browse and manage your files.
          </Text>
          <TouchableOpacity
      ...
      <ConfirmModal
        ...
        message="Are you sure you want to disconnect your OneDrive account? You will need to reconnect to access your files."
